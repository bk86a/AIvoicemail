...
Outputs (relative to [paths].generated_dir):
  ...
  asterisk/modules-cdr.conf      modules.conf include: noload cdr_csv.so when CDR is off (clean startup)
...
LOCAL_TEST_NET = "127.0.0.0/8"  # test-call's RTP comes from loopback; without it public_ip is advertised to it
HOST_GATEWAY = "host.docker.internal"  # compose extra_hosts: the Docker host as seen from the worker
BRIDGE_IFACES = ("docker0", "br-*")  # default bridge and compose networks
...
def pjsip_trunk(cfg) -> str:
    ...
    if t.public_ip and t.allow_local_test:
        out.append(f"local_net = {LOCAL_TEST_NET}")
...
def modules_cdr(cfg) -> str:
    """Included at the end of the [modules] section of asterisk/etc/modules.conf. With CDR off,
    cdr_csv would load, find CDR disabled and log a WARNING/ERROR on every start."""
    if cfg.retention.cdr:
        return f"{HEADER}\n; CDR on: cdr_csv.so stays loaded\n"
    return f"{HEADER}\n; CDR off ([retention].cdr = false)\nnoload = cdr_csv.so\n"


def host_gateway_ports(cfg) -> list[int]:
    """TCP ports of STT/LLM endpoints the worker reaches on the Docker host (e.g. Ollama at
    http://host.docker.internal:11434/v1). Such traffic arrives on a bridge interface and hits the
    input chain, so the firewall must accept it from the bridges (never from the outside)."""
    ports = set()
    for ep in (*cfg.stt.endpoints.values(), *cfg.llm.endpoints.values()):
        if not ep.base_url:
            continue
        try:
            ...
            raise ValueError(f"generate: refusing to interpolate invalid endpoint port in {ep.name}.base_url") from None
        if url.hostname == HOST_GATEWAY and port is not None:
            ports.add(_check_port(port, f"{ep.name}.base_url port"))
    return sorted(ports)
...
def nftables(cfg) -> str:
    ...
    out += [f'    iifname "{iface}" tcp dport {p} accept' for p in host_gateway_ports(cfg) for iface in BRIDGE_IFACES]
    if sig4:
...
def files(cfg) -> dict[str, str]:
    return {
        ...
        "asterisk/modules-cdr.conf": modules_cdr(cfg),
